...
スキップリンク（本文へジャンプ）、フォーカス可視化（`:focus-visible`）、テーブル見出しの
`scope="col"`、SVGグラフへの`role="img"`＋`<title>`（スクリーンリーダー対応）、ラジオボタン
グループの`<fieldset>/<legend>`化などの対応を行っている。正式なWCAG準拠監査は未実施。

### LGWAN本番閉域網相当のアクセス制限（ベストエフォート）

実際のLGWAN（総合行政ネットワーク）接続そのものはこの環境では再現できないため、近似として
IPアドレス／CIDRによるアクセス制限フィルタを実装している（admin-portal全体に適用）。

```bash
export ADMIN_IP_ALLOWLIST="10.0.0.0/8,192.168.1.100"
mvn -pl admin-portal exec:java
```

未設定（既定）の場合はアクセス制限を行わない。設定した場合、リストにないIPからのアクセスは
すべて403で拒否される（実機で確認済み）。本番でLGWAN接続を行う場合は、この疑似フィルタではなく
実際のLGWAN回線・行政共通のリバースプロキシ／WAF層でのネットワーク制御に置き換える前提。

## 非対応事項（本プロトタイプの現在のスコープ）

正式なアクセシビリティ監査（WCAG準拠確認）、本格的なペネトレーションテスト・WAF、多要素認証・
SSO/LDAP連携、実際のLGWAN接続（上記は近似的なIPアローリストのみ）などは未対応。詳細は改訂版要件定義書を参照。
